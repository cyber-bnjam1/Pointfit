...
// Filet de sécurité global : si le module plante, on affiche quand même l’auth
...
// Exposer sur window pour le onclick HTML ET le binding direct
...
return `<div class="food-db-item" onclick="loadFoodFromDB(${FOODS_DB.indexOf(f)})"> <div class="food-db-emoji">${f.emoji}</div> <div class="food-db-info"> <div class="food-db-name">${f.name}</div> <div class="food-db-meta">${f.cal} kcal · P:${f.prot}g · Sat:${f.sat}g · ${f.portion}g</div> </div> <div class="food-db-pts ${pts === 0 ? 'zero' : ''}">${pts === 0 ? '🟢 0' : pts} pts</div> </div>`;
}).join(’’);
};
...
list.innerHTML = filtered.map(item => `<div class="lib-item" onclick="loadFromLibrary('${item.id}')"> <div class="lib-item-emoji">${item.emoji || '🍽️'}</div> <div class="lib-item-info"> <div class="lib-item-name">${item.name}</div> <div style="font-size:0.72rem;color:var(--text2)">${item.cal || 0} kcal · P:${item.prot || 0}g · ${item.portion || 100}g</div> </div> <div class="lib-item-pts">${item.pts} pts</div> <button class="lib-item-del" onclick="event.stopPropagation();deleteLibItem('${item.id}')">🗑</button> </div>`).join(’’);
}
...
document.getElementById(‘weight-svg’).innerHTML = `<defs> <linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1"> <stop offset="0%"   stop-color="#FF6B35"/> <stop offset="100%" stop-color="#FF6B35" stop-opacity="0"/> </linearGradient> </defs> <polyline class="chart-line" points="${pts.join(' ')}"/> ${pts.map(p =>`<circle class="chart-dot" cx="${p.split(',')[0]}" cy="${p.split(',')[1]}" r="3"/>`).join('')} `;
}
